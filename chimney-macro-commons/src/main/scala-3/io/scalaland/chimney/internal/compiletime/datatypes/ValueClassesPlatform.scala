...
trait ValueClassesPlatform extends ValueClasses { this: DefinitionsPlatform =>
  ...
  protected object WrapperClassType extends WrapperClassTypeModule {
    ...
    def parse[A: Type]: Option[Existential[WrapperClass[A, *]]] = if !Type[A].isPrimitive then {
      ...
      val argument = paramListsOf(primaryConstructor).flatten match {
        ...
        case _ => assertionFailed(s"Wrapper/AnyVal ${Type.prettyPrint[A]} expected to have public constructor with 1 argument")
      }
