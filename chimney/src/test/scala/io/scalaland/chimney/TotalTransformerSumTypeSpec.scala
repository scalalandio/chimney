...
object TotalTransformerSumTypeSpec extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("not allow transformation of of sealed hierarchies when the transformation would be ambiguous") {
      ...
      error.check(
        "",
        "Chimney can't derive transformation from io.scalaland.chimney.examples.shapes1.Shape to io.scalaland.chimney.examples.shapes5.Shape",
        "io.scalaland.chimney.examples.shapes5.Shape",
    ...
    test("setting .withCoproductInstance[Subtype](mapping)") {
      ...
      test(
        ...
        compileError("""(colors2.Black: colors2.Color).transformInto[colors1.Color]""").check(
          "",
