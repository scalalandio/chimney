...
/** Scala 2-only: `val` members of structural refinement bounds are readable accessors OUT OF THE BOX (no
  ...
  *
  * Regression test for the Hearth migration: scalac represents `val value: String` of a refinement as a deferred
  * STABLE method with no accessed field, which Hearth 0.4.0's `Method.isVal` does not count as a val - without
