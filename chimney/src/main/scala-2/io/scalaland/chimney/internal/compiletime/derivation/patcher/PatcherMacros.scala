...
final class PatcherMacros(val c: blackbox.Context) extends DerivationPlatform with Gateway {
  ...
  def derivePatchImpl[A: WeakTypeTag, Patch: WeakTypeTag, C <: PatcherCfg: WeakTypeTag]: c.Expr[A] =
    cacheDefinition(c.Expr[dsl.PatcherUsing[A, Patch, C]](c.prefix.tree)) { pu =>
      Expr.block(
