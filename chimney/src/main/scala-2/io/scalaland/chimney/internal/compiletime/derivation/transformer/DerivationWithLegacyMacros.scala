...
import io.scalaland.chimney.internal.compiletime.{ConfigurationsPlatform, Contexts, DefinitionsPlatform, DerivationError, DerivationErrors, DerivationResult}
...
private[compiletime] trait DerivationWithLegacyMacros {
  ...
  private def deriveTransformerTargetExprWithOldMacros[From, To](implicit
      ctx: TransformerContext[From, To]
  ...
  protected val legacyMacrosRule: Rule = new Rule {
    ...
    override def apply[From, To](implicit ctx: TransformerContext[From, To]): DerivationResult[DerivedExpr[To]] =
      deriveTransformerTargetExprWithOldMacros
  }
}
