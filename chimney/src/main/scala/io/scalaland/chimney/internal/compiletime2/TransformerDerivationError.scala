...
import io.scalaland.chimney.internal.compiletime2.datatypes.ProductTypes.BeanAware

/** Transformer-specific error related to derivation logic.
  *
  * Hearth-based port of `io.scalaland.chimney.internal.compiletime.TransformerDerivationError` - 1:1 copy (all case
  * classes, fields and `printErrors` rendering preserved byte-identical), only the `BeanAware` import points at the
  * `compiletime2` datatypes.
  */
