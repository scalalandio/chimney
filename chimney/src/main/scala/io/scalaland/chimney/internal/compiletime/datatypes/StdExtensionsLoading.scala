...
/** Loads Hearth's standard extensions ([[hearth.std.StdExtensions]] providers: `IsCollection`, `IsMap`, `IsOption`,
  ...
  * call must not be repeated within one expansion (redundant `ServiceLoader` scans + "already loaded" info logs). The
  * `var` lives on this trait, which is mixed exactly once into the final macro-bundle class, making the guard
  * per-expansion.
  *
  * The datatypes adapters themselves deliberately do NOT depend on standard extensions (see their ScalaDocs) - this
  * hook exists for the Gateway/rules layer.
  */
private[compiletime] trait StdExtensionsLoading { this: hearth.MacroCommons & hearth.std.StdExtensions =>
