...
trait TransformPartialOptionToNonOptionRuleModule { this: Derivation =>

  import TypeImplicits.*, ChimneyTypeImplicits.*

  object TransformPartialOptionToNonOptionRule extends Rule("PartialOptionToNonOption") {

    def expand[From, To](implicit ctx: TransformerContext[From, To]): DerivationResult[Rule.ExpansionResult[To]] =
      (ctx, Type[From]) match {
        case (_: TransformerContext.ForPartial[?, ?], Type.Option(from2)) if !(Type[To] <:< Type[Option[Any]]) =>
          ComputedType.use(from2) { implicit InnerFrom: Type[from2.Underlying] =>
            ...
            DerivationResult
              .direct { (await: DerivationResult.Await[DerivedExpr[To]]) =>
