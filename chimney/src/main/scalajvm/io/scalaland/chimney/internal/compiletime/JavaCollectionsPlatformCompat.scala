...
/** JVM implementation of the `java.util.EnumSet`/`java.util.EnumMap` compat for the Hearth-provider fallback layer (see
  ...
  *
  * HEARTH 0.4.0 BUG WORKAROUND (hearth#324): `IsCollectionProviderForJavaCollection`'s EnumSet branch and
  ...
  *
  * The fix (never exclude a provider - fix its bug in compat): the fallback layer REPLACES the provider's `factory`
  * expr for EnumSet/EnumMap-shaped targets with a Chimney-built equivalent whose class token is a plain class LITERAL
  * of the concrete enum type ([[MacroCommonsCompat.classOfExprCompat]] - prints as `classOf[fqcn.Enum]` and
  * re-typechecks on both Scala versions). The generated factories mirror Hearth's own (reflective `EnumSet.noneOf` /
  * `new EnumMap[Nothing, V](cast)` tricks bypassing the `E <: Enum[E]` bounds that an unbounded macro type parameter
  * cannot satisfy). Applied on BOTH Scala versions, so the single replacement code path is exercised by both test
  * suites.
  *
  * RELATED HEARTH 0.4.0 LIMITATIONS (hearth#323, pinned in HearthStdJavaTypesSpec):
...
private[compiletime] trait JavaCollectionsPlatformCompat {
  ...
  /** `Iterator[Item]` for an `M <: java.util.Collection[Item]` source (hearth#322) - replacement for the EnumSet
    * provider branch's poisoned `asIterable` (its quote is written INLINE in the provider's `parse` with the
    * existential-imported `Item`, so its generated Scala 2 tree references the provider's local `item` value - same
    * "not found: value item" family as the class token; the other collection branches route their quotes through helper
    * methods with regular type parameters and are fine).
    */
  @scala.annotation.nowarn("msg=is never used")
  protected def javaCollectionIteratorCompat[Item: Type, M: Type](collection: Expr[M]): Expr[Iterator[Item]] = {
    implicit val IteratorItem: Type[Iterator[Item]] = Type.of[Iterator[Item]]
    Expr.quote {
  ...
  /** `Iterator[(K, V)]` for an `M <: java.util.Map[K, V]` source (hearth#322) - replacement for the EnumMap provider
    * branch's poisoned `asIterable`/`key`/`value` (see [[javaCollectionIteratorCompat]] - the EnumMap branch's quotes
    * suffer the same inline-quote problem). Bypasses the provider's `java.util.Map.Entry` pair type entirely.
    */
  @scala.annotation.nowarn("msg=is never used")
  ...
  /** `Factory[Item, M]` for an `M =:= java.util.EnumSet[Item]` target (hearth#324) - replacement for the provider's
    * poisoned factory (see the trait ScalaDoc). Mirrors Hearth's generated code shape.
    */
  @scala.annotation.nowarn("msg=is never used")
  ...
  /** `Factory[(K, V), M]` for an `M =:= java.util.EnumMap[K, V]` target (hearth#324) - replacement for the provider's
    * poisoned pair factory. Built directly at the TUPLE level (the engine's Map contract), so the provider's
    * `java.util.Map.Entry` pair type is not involved at all on the construction side.
    */
  @scala.annotation.nowarn("msg=is never used")
